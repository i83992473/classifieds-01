...
for ($retry = 1; $retry -le $MaxBuildRetries; $retry++) {
    if ($retry -gt 1) {
        Write-Host "" 
        Write-Host "Retrying build ($retry of $MaxBuildRetries)..." -ForegroundColor Yellow
    }
    ...
    while ($attempts -lt $MaxAttempts) {
        ...
        try {
            ...
            if ($status -eq "FAILED") {
                ...
                if ($retry -lt $MaxBuildRetries) {
                    Write-Host "Cleaning up failed job before retry..." -ForegroundColor Yellow
                    try {
                        aws amplify delete-job --app-id $AppId --branch-name $BranchName --job-id $jobId --region $Region --profile $AwsProfile 2>&1 | Out-Null
                    } catch {
                        Write-Host "[WARN] Failed to delete job: $_" -ForegroundColor Yellow
                    }

                    Write-Host "Waiting $RetryDelaySeconds seconds before retry..." -ForegroundColor Yellow
                    Start-Sleep -Seconds $RetryDelaySeconds

                    $startJobJson = aws amplify start-job --app-id $AppId --branch-name $BranchName --region $Region --job-type RETRY --output json --profile $AwsProfile 2>&1
                    if ($LASTEXITCODE -ne 0) {
                        Write-Host "[ERROR] Failed to retry build: $startJobJson" -ForegroundColor Red
                        exit 1
                    }
